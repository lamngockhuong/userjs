...
<template>
  <article
    class="group p-4 rounded-lg border cursor-pointer transition-colors duration-200 overflow-hidden
    ...
    <div class="mt-4 flex flex-wrap gap-2">
      ...
      <button @click.stop.prevent="copyUrl(script.installUrl)"
        ...
        <span class="hidden sm:inline">{{ copied ? 'Copied!' : copyError ? 'Failed' : 'Copy URL' }}</span>
        <span class="sm:hidden">{{ copied ? 'Copied!' : copyError ? 'Failed' : 'Copy' }}</span>
      </button>
