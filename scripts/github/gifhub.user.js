...
(function () {
  ...
  // Fetch image via GM_xmlhttpRequest to bypass CSP and return blob URL
  ...
  function closeModal() {
    ...
    if (modal) {
      ...
      modal.querySelectorAll('.gif-picker-item').forEach(item => {
        if (item._blobUrl) {
  ...
  function renderGifItem(gif, query, container) {
    ...
    // Fetch image via GM_xmlhttpRequest to bypass CSP
    fetchImageAsBlob(previewUrl)
      .then(blobUrl => {
        img.src = blobUrl;
