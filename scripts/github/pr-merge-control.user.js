...
(function () {
  ...
  // Get target branch from PR page
  ...
  // Apply inline styles directly (bypass CSS specificity issues)
  function disableItem(item) {
    ...
    // Add disabled badge to label
    const label = item.querySelector('[class*="ItemLabel"]');
    if (label && !label.querySelector('.pr-merge-disabled-badge')) {
  ...
  function enableItem(item) {
    ...
    const badge = item.querySelector('.pr-merge-disabled-badge');
    if (badge) badge.remove();
  }

  function applyMergeRestrictions(menu, targetBranch) {
    ...
    items.forEach((item) => {
      const label = item.querySelector('[class*="ItemLabel"]')?.textContent?.trim();
      if (!label) return;

      // Reset state first
      enableItem(item);

      const isMergeCommit = label.toLowerCase().includes('merge commit');
      const isSquash = label.toLowerCase().includes('squash');
      const isRebase = label.toLowerCase().includes('rebase');

      if (isRelease) {
        // Release branch: only allow merge commit
        if (isSquash || isRebase) {
          disableItem(item);
        }
      } else {
        // Feature branch: only allow squash
        if (isMergeCommit || isRebase) {
          disableItem(item);
        }
      }
    });
    ...
    console.log(
      `[PR Merge Control] Target: ${targetBranch} (${isRelease ? 'release' : 'feature'} branch)`
    );
  }

  function observeDropdownMenu() {
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          if (node.nodeType !== Node.ELEMENT_NODE) continue;

          // Find menu in added node or its children
          const menu =
          ...
          if (menu && menu.querySelector('li[aria-keyshortcuts="c"]')) {
            ...
            if (targetBranch) {
              ...
              setTimeout(() => applyMergeRestrictions(menu, targetBranch), 10);
            }
          }
        }
      }
  ...
  function init() {
    observeDropdownMenu();
    console.log('[PR Merge Control] v1.1 Initialized');
  }
