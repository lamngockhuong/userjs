...
(function () {
  ...
  const STYLE_ID = 'pr-merge-control-styles';
  ...
  function getTargetBranch() {
    ...
    // Fallback: look for base branch in merge info
  ...
  // Apply merge restrictions to dropdown menu
  function applyMergeRestrictions(menu, targetBranch) {
    ...
    items.forEach((item) => {
      const label = item.querySelector('.prc-ActionList-ItemLabel-81ohH')?.textContent?.trim();
      if (!label) return;

      // Remove previous disabled state
      item.classList.remove('pr-merge-disabled');

      const isMergeCommit = label.includes('merge commit');
      ...
      if (isRelease) {
        ...
        if (isSquash || isRebase) {
          item.classList.add('pr-merge-disabled');
        }
      } else {
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          ...
          // Check if it's the merge dropdown menu
          const menu = node.querySelector?.('ul[role="menu"]') || (node.matches?.('ul[role="menu"]') ? node : null);

          if (menu && menu.querySelector('li[aria-keyshortcuts="c"]')) {
            const targetBranch = getTargetBranch();
            if (targetBranch) {
              applyMergeRestrictions(menu, targetBranch);
            }
          }
        }
      }
  ...
  function init() {
    injectStyles();
    observeDropdownMenu();
    console.log('[PR Merge Control] Initialized');
  }
