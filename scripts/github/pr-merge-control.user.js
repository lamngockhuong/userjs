...
(function () {
  ...
  function applyMainButtonRestriction(targetBranch) {
    // Find all buttons with merge-related text
    const allButtons = document.querySelectorAll('button.flex-1[data-variant="primary"]');

    for (const mainButton of allButtons) {
      const labelEl = mainButton.querySelector('[data-component="text"]');
      const label = labelEl?.textContent?.trim();
      if (!label) continue;

      // Check if this is a merge-related button
      const lowerLabel = label.toLowerCase();
      if (!lowerLabel.includes('merge') && !lowerLabel.includes('squash') && !lowerLabel.includes('rebase')) {
        continue;
      }
  ...
  function pollForMergeBox(maxAttempts = 30, interval = 500) {
    ...
    const poll = () => {
      ...
      const mainButton = document.querySelector('button.flex-1[data-variant="primary"]');
  ...
  function init() {
    observeDOM();
    pollForMergeBox();
    console.log('[PR Merge Control] v1.4 Initialized');
  }
